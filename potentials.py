...
def potential_wnt_nematic(x, d, dx, lam_i, lam_j, pi, pj, qi, qj, wnt_cells, wnt_range):
    G = x[:, None, :] - x[wnt_cells] # displacements from each cell to each WNT-producing cell. shape: n_cells x n_wnt_cells x 3
    GG = G.clone()
    ...
    S2 = torch.abs(torch.sum(torch.cross(pi, qi, dim=2) * torch.cross(pj, qj, dim=2), dim=2))
    S3 = torch.abs(torch.sum(torch.cross(qi, dx, dim=2) * torch.cross(qj, dx, dim=2), dim=2))
    S4 = torch.sum(torch.sum(torch.cross(qi[:,0,:][:,None,:].expand(qi.shape[0], G.shape[1], 3), GG, dim = 2)**2, dim = 2), dim = 1)
    ...
    S = lam[:, :, 0] + lam[:, :, 1] * S1 + lam[:, :, 2] * S2 + lam[:, :, 3] * S3 + lam[:, :, 4] * S4[:, None].expand(S4.shape[0], lam.shape[1])
    Vij = torch.exp(-d) - S * torch.exp(-d / 5)
    return Vij

    
def potential_wnt(x, d, dx, lam_i, lam_j, pi, pj, qi, qj, wnt_cells, wnt_range):
    G = x[:, None, :] - x[wnt_cells] # displacements from each cell to each WNT-producing cell
    G = x[:, None, :] - x[wnt_cells] # displacements from each cell to each WNT-producing cell. shape: n_cells x n_wnt_cells x 3
    GG = G.clone()
    Gnorm = torch.sqrt(torch.sum(G**2, dim = 2))
    GG[Gnorm > wnt_range] = 0
    ...
    S2 = torch.sum(torch.cross(pi, qi, dim=2) * torch.cross(pj, qj, dim=2), dim=2)
    S3 = torch.sum(torch.cross(qi, dx, dim=2) * torch.cross(qj, dx, dim=2), dim=2)
    S4 = torch.sum(torch.sum(torch.cross(qi[:,0,:][:,None,:].expand(qi.shape[0], G.shape[1], 3), G, dim = 2)**2, dim = 2), dim = 1)
    ...
    S = lam[:, :, 0] + lam[:, :, 1] * S1 + lam[:, :, 2] * S2 + lam[:, :, 3] * S3 + lam[:, :, 4] * S4
    Vij = torch.exp(-d) - S * torch.exp(-d / 5)
    return Vij